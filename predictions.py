...
def load_models():
    ...
    if not gender_model_path.exists() or not age_model_path.exists():
        raise RuntimeError(f"Model files not found in {models_dir}. Expected: {gender_model_path.name}, {age_model_path.name}")
    
    # loading pretrained models
    try:
        gender_model = tf.keras.models.load_model(str(gender_model_path))
...
def analyze_faces(img, age_model, gender_model):
    ...
    if detections:
        ...
        age_preds = age_model.predict(face_imgs)
        gender_preds_prob = gender_model.predict(face_imgs)
    ...
    if uploaded_file is not None:
        ...
        if st.button("Analyze"):
            with st.spinner('Processing...'):
                detections, predictions = analyze_faces(image, age_model, gender_model)
            if detections:
